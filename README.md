...
*   **flags 可修改项**:
    ...
    *   `critThresholdMod`**: **暴击阈值修正**。正数表示更容易暴击（即阈值降低）。例如 `+2` 表示 18 即可暴击。
...
*   **flags 可修改项**: 
    *   `level` (自身命中修正)
    *   `feintLevel` (虚招修正)
    *   `critThresholdMod`**: **全局暴击阈值修正**。此次出招对所有目标的暴击阈值修正。正数表示更容易暴击。
    *   `abort` (阻断)
...
#### J. 针对破绽暴击 (如：攻其不备)
> **场景**: 被动/Buff (Check 脚本)。如果目标处于“晕眩”或“破绽”状态，对其暴击门槛降低。
> **触发时机**: `check` (对每个目标运行)
