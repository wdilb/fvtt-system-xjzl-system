...
#### 4. 双虚招对抗 (doubleFeint)
少数虚招招式需要**进行两轮虚招对抗**（如“日月同天”）。虚招招式上配置两个字段即可启用，未配置时走原有单轮逻辑、行为完全不变：

*   **`doubleFeint: true`**：开启双轮对抗。施展后系统会对每个目标**发两张看破请求卡**（两轮各自独立投骰），并在应用伤害时**合并判定**破架结果。
*   **`doubleFeintMode`**（仅 `doubleFeint: true` 时生效）：
    *   `"both"`（默认）：**两轮都击破**才算击破招架；两轮都被看破时目标才可反击。
    *   `"any"`：任意一轮击破即算击破招架。
...
| `damageOnFail` / `damageOnSuccess` | Object | **(可选)** 失败/成功时的后果。结构: `{ value: 10, type: "..." }`。<br>• 若 `type` 为资源 (`"hp"`, `"mp"`, `"tili"`, `"rage"`)：**直接流失**，无视护甲抗性。<br>• 若 `type` 为伤害 (`"poison"`, `"waigong"`)：**造成伤害**，正常计算目标抗性和护体抵扣。 |
