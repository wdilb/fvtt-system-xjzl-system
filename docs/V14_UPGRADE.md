...
1. **ActiveEffect 融合**（§2）：V14 把 `changes` 迁入 `system.changes`、数字 `mode` 改字符串 `type`、duration 模型重做，并支持独立 AE 文档与拖放。保留现有叠层、抑制、脚本、提示和时长业务规则，按 [`V14_AE_PLAN.md`](V14_AE_PLAN.md) 接入新机制。
2. **距离与光环 / Region**（§3）：MeasuredTemplate 文档类型已弃用（14.368 保留兼容层），AOE 工具与跟随光环改用 Region 体系重建；Q4 已定稿为完整光环系统（不新增脚本触发器），按 [`V14_AURA_PLAN.md`](V14_AURA_PLAN.md) 实施。
...
- Token 属性覆写：AE 变更的 `key` 可使用 `token.` 前缀，核心把它应用到 Actor 的 Token 覆写数据，可改视野、光照、形象、阵营、透明度等；核心不直接处理 Token 网格尺寸。本次核对兼容，不迁移原有脚本来强行采用新写法。
- `showIcon`：`NEVER=0`、`CONDITIONAL=1`、`ALWAYS=2`，默认 `CONDITIONAL` 只显示临时 AE。系统以 `addEffect` 直接创建的永久通用状态若未显式指定，Token/战斗追踪器图标可能消失；按 AE-13 为应常显的状态明确设置 `ALWAYS`。
- ActiveEffect 升级为可直接存入合集包的文档类型。
- `ActiveEffect#origin` 改为允许相对 UUID 的 DocumentUUIDField；拖放复制时保留原格式合法的来源，即使来源文档暂时无法解析也不擅自替换，缺失时才按源 Item/AE 补来源，避免架招豁免逻辑失效。
...
| 过期清理（战斗流转时 `cleanExpiredEffects`） | `xjzl-system.mjs` updateCombat 流程 | expiry 事件 + registry | 按 Q5 优先由核心清理，撤下系统重复扫描；确保结果正确并保留公开兼容语义 |
| 状态选取器（通用状态/场上特效/收藏/最近） | `effect-selection-dialog.mjs` | AE 仍可来自现有通用状态或内容物品 | 对象访问替换已由 S1.5 完成；S2.8 保留并联调现有分类、目标和交互，不强制新建合集包分类 |
...
- duration：`ActiveEffectManager.addEffect` 中 `rounds/turns/seconds/startRound/startTurn/startTime` 的读写、叠加与刷新逻辑按 V14 schema 适配；开始锚点移到顶层 `start`。`compareDurations`/`getDurationScore` 保留“只接受不短于现有效果的时长”业务规则，改按新单位与 expiry 事件比较，不能仅替换字段名。从已生效 AE 拖放复制时保留原 `duration`/`start`，不额外重置。Q5 只要求到期清理正确、不重复，核心与回合脚本的先后不作门槛。
...
- **`XJZLActiveEffectConfig`**（[active-effect-config.mjs](../module/sheets/active-effect-config.mjs)）：现依赖 `renderXJZLActiveEffectConfig` 钩子 + jQuery 注入 DOM + `app._tabs[0].active` 内部状态 + `app.setPosition` + V13 核心配置窗 Tab 结构。V14 核心 ActiveEffectConfig 已使用 `PARTS`/`TABS`/`actions`。迁移时保留核心详情、时长、变更三页和 `showIcon` 等新字段，并加入系统自己的“侠界配置”页；原有 slug、叠层开关/上限、`tiedToStance`、脚本标签/时机/启停/代码及变更 key 自动补全都要能编辑和保存，详见专项计划 AE-07。
...
- 固定的通用 AE 模板日后可另建独立合集包；内容专属 AE 继续随武学、内功、物品维护，本次不建立需与这些内容同步的第二份 AE 目录，也不强制把独立 AE 包接入状态选取器。
...
> Q4 已于 2026-09-26 定稿为方案 B（完整光环系统），实施框架、决策记录与工作项（AURA-01～12）移至 [`V14_AURA_PLAN.md`](V14_AURA_PLAN.md)；本节保留候选架构与脚本引擎集成设想供追溯，不代表待办状态。定稿时未采纳下方"新触发器"设想——按决策不新增脚本触发器，复杂规则以监听 AE 复用既有触发器实现。
...
总里程碑进度与验收记录在执行计划维护；AE 细项与证据在 AE 专项计划维护并同步总里程碑。本表定义完成条件。工作项编号用于稳定引用，不代表必须按数字顺序执行。
...
| M2 AE 链路 | 直接按 [`V14_AE_PLAN.md`](V14_AE_PLAN.md) 执行 AE-01–AE-09、AE-12 的迁移框架及 AE-13；源数据和旧世界验收在 M4 | V14 测试数据验证旧功能、到期清理结果及不重复执行、数据格式、独立 AE 文档、物品/角色/Token 拖放和状态选取器既有功能；不以新建合集包目录为验收条件 |
| M3 Region/光环 | 恢复旧 AOE 功能与距离语义；Q4 已定稿纳入完整光环（不新增触发器），按 [`V14_AURA_PLAN.md`](V14_AURA_PLAN.md) 实施 | 框架出口（AURA-01～06+08）：静态/跟随区域、Token 删除清理、1-2-2-2 判定与进出结算在测试世界通过；内容批次出口见专项计划 |
...
优先做 M0 的机制试验，可使用独立 V14 最小环境；若验证必须依赖完整系统，先完成 S1.14 等必要启动修复，再补做依赖它的验证，不把暂时无法验证记作通过。M1 启动可用后，M2/M3 可穿插；M3 按光环专项计划实施（Q4 已定稿）。M4 先完成源数据、脚本及 API 审计，再执行 S4.4 重建合集与 S4.7 世界迁移验收；状态查询迁移依赖 S2.9。M0 中的机制结论不能替代 M1–M5 的系统集成验收。
