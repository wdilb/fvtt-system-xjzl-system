...
**当前下一步：M2 进行中。AE-01（文档层）、AE-02（全链路格式迁移与门面归一化）、AE-03（getStatus）、AE-04（时长引擎）已完成并经四轮审阅修正（提交 `4ad4da7`，14.368 实测通过，详见专项计划各条记录）；下一步按专项计划实施顺序做 AE-05（核心 registry 过期清理 `expiryAction:"delete"` + 撤下旧 updateCombat 扫描 + 战斗内联调），随后 AE-06/AE-07/AE-08/AE-09 与 AE-12 迁移框架，AE-13 收尾装备被动图标核对。M1 已验收通过；到期顺序不再是前置决策，机制结论见 M0 记录。**
