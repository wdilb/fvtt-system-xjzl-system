...
**当前下一步：M2 进行中。AE-01～09 已完成（AE-01～06：提交 `4ad4da7`、`6a1fde8`；AE-07～09：Foundry 14.368 实测，详见专项计划）。剩余 AE 工作：先收尾 AE-13（"所有非被动施加型常显"——经 V13/V14 源码对照确认这是相对 V13 的有意行为变更，实施以数据侧补设为主、门面仅兜底，见专项计划）与 AE-12（世界迁移框架，只执行一次）以达成 M2 出口；随后进入 M4 按 AE-10（源 JSON 与脚本迁新写法，含脚本内联旧入参、模板 showIcon 补设与 seed 透传）→ AE-11（重建合集包，前置含非 AE 的 S4.8 审计）→ AE-14（旧世界副本验收）。M1 已验收通过；到期顺序的机制结论见 M0 记录。**
