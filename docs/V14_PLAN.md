...
**当前进度**：M0～M2 已通过；M4 的源数据与合集包已通过，AE-14 首跑演练完成，正式迁移待执行。M3 专项计划已定稿（[`V14_AURA_PLAN.md`](V14_AURA_PLAN.md) 第 7 节，Q4/S3.6 已关闭），AURA-01～06 已全部完成（2026-09-26，14.368 实机验收：机制 spike、形状生成器、行为类型/管理器/传送对账三件套、快建工具；验收中发现 9 项问题当场修复并回归，明细见专项计划第 7.2 节修复 ⑳～㉙）；AURA-08 闸门的账本主测用例已随验收先行通过，样板批次与 AURA-09～12 待推进；M5 尚未完成。

**下一步**：正式迁移前备份世界，在 V13 运行 [`v13-ae-preprocess.js`](migration/v13-ae-preprocess.js) 并确认报告中的 `unverified`、`errors` 均为 0，再按 AE-14 清单验收旧特效与持久化；M3 推进 AURA-08 样板闸门（5 条样板＋专用测试物品，账本用例已先行验证）与 AURA-09～11 内容批次，AURA-07 粒子保持可选；完成 M5 回归及文档核对。S5.5 的 AE 文档已随 AE-10 更新。本轮回归结果及待处理项见文末记录。
...
- [x] S4.3 `data/macros/utility.json` 光环宏改写——已并入 AURA-06 并完成（改写为指向快建工具的说明宏，未删除，`game.xjzl.seed.macros()` 重建后合集包内生效）
...
| M3 | **框架实机验收通过（AURA-01～06 完成）** | 2026-09-26 构建 14.368 实机验收（浏览器自动化，GM＋双客户端）：AURA-01 机制 spike（区域事件派发口径、跟随与源删除、性能 921 格、矩形旋转）；AURA-02 形状生成器＋Node 断言；AURA-03/04/05 框架三件套——进出结算、贡献账本（双光环 1→2→1→移除、预存、满层钳制、清空型、串行交错）、阵营过滤、分档 payload、直接动作事务、回合结算与 round 门控、节流、moveWithin、时限/维持/脱战/孤儿校验/生命周期三层（装备/运功/架招）、强制失配对账（补挂/补摘/零操作）、socketlib 委托双客户端全链路（玩家端触发→GM 端单端结算）、queryTokens 足迹口径、配置页改半径原地生效、无网格禁用；AURA-06 快建工具（按钮/弹窗/格心吸附放置/自绘显示名标签/mark 零结算/旧宏改写＋seed 重建）。验收中发现 9 项问题当场修复并回归（账本惰性求值、并行创建竞态、删除补发 exit 不可靠、账本写回守卫、遗留删除语法、维持扣取双缺陷、PIXI v7 API、mark 默认开关、V14 工具栏结构等），明细见专项计划第 7.2 节修复 ⑳～㉙。测试数据已全部清理。AURA-07 粒子未实施（保持候选）；AURA-08～12 待推进 |
