...
**当前进度**：M0～M2 已通过；M4 的源数据与合集包已通过，AE-14 首跑演练完成，正式迁移待执行。M3、M5 尚未完成。

**下一步**：正式迁移前备份世界，在 V13 运行 [`v13-ae-preprocess.js`](migration/v13-ae-preprocess.js) 并确认报告中的 `unverified`、`errors` 均为 0，再按 AE-14 清单验收旧特效与持久化；S3.6 确定 Region/光环范围后推进 M3；完成 M5 回归及文档核对。S5.5 的 AE 文档已随 AE-10 更新。本轮回归结果及待处理项见文末记录。
