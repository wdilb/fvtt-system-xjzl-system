...
**当前下一步：AE 专项计划的源数据/合集包/代码链路工作已完成并实机验收（AE-01～13；AE-14 真实世界验收移交用户手动，见文末 M4 记录）。剩余工作：①用户手动项——AE-14/S4.7 真实 V13 世界副本迁移验收（按专项计划 AE-14 附带的 9 步清单，含升级前 V13 侧预处理；迁移对时长 rounds+turns 并存数据存在已知不可恢复损失，须先预处理；在正式世界升级时执行或用 v13data 副本演练）；②M3 Region/光环——入口是 S3.6 的 Q4 产品范围决策（完整光环 vs 仅恢复旧 AOE），需用户定夺后再排实施；③M5 回归与文档——完整回归依赖 M3 完成与 S4.7 手动验收，其中 S5.5 文档同步的 AE 部分已随 AE-10 提前完成（SCRIPT_ENGINE.md/SEEDING_GUIDELINES.md）。M0/M1/M2 已验收通过；M4 的源数据与合集包范围验收通过（S4.3 暂缓、S4.7 移交，世界迁移的最终完成表述待 AE-14 验收后使用）。**
...
- [ ] S4.3 `data/macros/utility.json` 光环宏改写（或并入 S3.6）——暂缓：依赖 M3 光环架构决策（Q4），不阻塞 M4 的 AE 出口
...
| M3 | 待验 | — |
| M4 | **源数据/合集包通过；世界迁移待 AE-14 手动验收** | 2026-09-25 构建 14.368 实机验收：S4.1/S4.2/S4.6 随 AE-10 完成（data/ 源 JSON 全量迁 V14 新写法：2378 条模板 effects、516 处状态查询、约 750 处内联 duration、71 处 mode、169 处 changes 读取，4613 个脚本语法回归通过）；S4.4 随 AE-11 完成（11 类合集包重建、1438 个施加型模板 showIcon:2 透传、端到端常显画布确认）；S4.5 完成（2 处 measureDistance → measurePath，euclidean/cost 语义与 V13 逐一对等）；S4.8 审计完成（user: 0 残留、2 处旧字段读取修复、其余命中均为存活 API）。S4.3 暂缓（依赖 M3 Q4 决策）；S4.7 移交用户手动（AE-14 附 9 步验收清单，含升级前 V13 侧 rounds+turns 预处理——迁移器工作于核心清洗后的 _source，并存时长形态的损失须预处理规避；迁移转换与版本机制已在 M2 实测，真实存档的时长恢复由本验收判定）。详见专项计划 AE-10/AE-11/AE-14 记录 |
| M5 | 待验 | — |
