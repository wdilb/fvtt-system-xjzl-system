...
**当前下一步：M2 进行中。AE-01～04 已完成并经四轮审阅修正（提交 `4ad4da7`）；AE-05（registry `expiryAction:"delete"` 过期清理 + 撤下旧扫描）、AE-06（架招绑定判定与业务清理回归）、AE-07（AE 编辑器迁 ApplicationV2）、AE-08（六类模板可拖 AE 行 + 角色/画布接收走门面 + 独立合集包互操作）已完成 14.368 实测（详见专项计划各条记录）；下一步按专项计划实施顺序做 AE-09（状态选取器与 TokenHUD/时长标签回归），随后 AE-12 迁移框架，AE-13 收尾装备被动图标核对。M1 已验收通过；到期顺序不再是前置决策，机制结论见 M0 记录。**
